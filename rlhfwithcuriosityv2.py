...
OUTPUT_DIR = "/workspace/rlhf-demo"
...
for step in trange(PPO_UPDATES):
    ...
    queries = batch["query"]
    inputs = actor_tokenizer(queries, padding=True, truncation=True, max_length=MAX_PROMPT_LEN, return_tensors="pt").to(actor_model.device)
    responses = ppo_trainer.generate(inputs["input_ids"], **gen_kwargs)
    ...
    stats = ppo_trainer.step(inputs["input_ids"], responses, torch.tensor(rewards).to(device))
