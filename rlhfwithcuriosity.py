...
# === Constants and Paths ===
ACTOR_MODEL_NAME = "meta-llama/Llama-3.2-1B"  # Original model
...
# Patch the transformers library to handle RoPE configuration
...
# Create a patched version that accepts the new RoPE format
def patched_rope_validation(self):
    if self.rope_scaling is not None:
        if isinstance(self.rope_scaling, dict):
            # Accept the new format with rope_type
            ...
            # If it's already in the old format, keep it
            elif 'type' not in self.rope_scaling:
                raise ValueError("rope_scaling must have 'type' field")

# Apply the patch
llama_config.LlamaConfig._rope_scaling_validation = patched_rope_validation

try:
    # Now load the model with the patched configuration
    ...
    if torch.cuda.is_available():
        ...
        # Don't use device_map="auto" on GPU, let it load normally
        actor_model = AutoModelForCausalLMWithValueHead.from_pretrained(
            ACTOR_MODEL_NAME,
            torch_dtype=torch_dtype,
            device_map=None  # Let it load to GPU normally
        ).to(device)
    ...
    print("Successfully loaded Llama-3.2-1B with RoPE patch!")

except Exception as e:
    print(f"Failed to load with patch: {e}")
    print("Restoring original validation method...")
